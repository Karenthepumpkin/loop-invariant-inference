...
**Diagnosis — symexec passes but admits remain**: When `grep -c "Admitted\."` returns `> 0` but there are zero fatal errors, the invariant is well-formed (symbolic execution completed) but some proof obligations cannot be discharged by strategies. This pattern has a single root cause in most cases:
- All four witness types admit together (`entail_wit_1` + `entail_wit_2` + `return_wit_1` + `return_wit_2`) → likely a **redundant field reference in PROP**. A field-expression equality like `u == t->next` or `t->next == u` duplicates information already encoded by a SEP predicate (`dllseg_shape(x, 0, t, u)` already implies `t->next == u`). Remove the field-expression PROP and re-evaluate.
- Only one or two admit → check the specific witness type against the diagnosis table above. `entail_wit_1` alone → invariant entry proof fails. `entail_wit_2` alone → loop preservation fails. `return_wit` alone → invariant + exit condition doesn't imply Ensure.

**How to apply fix strategies**: Apply strategies ONE AT A TIME. If a change eliminates a fatal error but introduces Admitted witness goals, REVERT IT before trying the next strategy — do not stack changes on top of a partial fix. A change that trades a fatal error for admits is not an improvement; the admits signal that the change introduced a new problem (typically a redundant field reference via a field-expression PROP).
...
2. **Replace field expressions with `exists` variables**: If `dup_data_at_error` occurs, check Cause 1 above. Replace the conflicting C field expression with an `exists` variable: `exists p_prev, dllseg_shape(l, prev, p_prev, p)`.

3. **Widen the boundary with field expressions on both sides**: If `entail_wit_2` or `return_wit` fail, the boundary between `dllseg` and `dlistrep` may be too tight for strategies to fold/unfold. Use field expressions on BOTH sides to explicitly expose the gap node's fields — the boundary becomes self-adapting and no strategy rule is needed at all.
...
4. **Add non-null PROP to trigger unfolds**: If entail or safety witnesses fail after symexec passes, strategies may need explicit non-null constraints to fire unfold rules for shape predicates. Add `v != 0` where `v` is the variable whose node's fields the loop body (or post-loop code) dereferences or assigns through. Do NOT add field-expression equalities like `t->next == u` — use simple pointer-non-null constraints only. Before trying this strategy, first remove any field-expression PROP already present (see Cause 2 of `dup_data_at_error` above) — a field-expression PROP and a SEP predicate encoding the same relationship create redundant `data_at` that cause silent admit failures.

5. **Add `has_permission`**: Re-read the 2.6 checklist.
